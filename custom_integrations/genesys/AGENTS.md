...
2. **A dashboard's `isPrivate: false` in the YAML does not make it visible to others.** Visibility is a separate share action: `dtctl apply -f dashboard.yaml --share-environment read` (share with everyone in the tenant) or `dtctl share dashboard <id> --user/--group ...` (specific people). Don't assume the YAML field alone did it — check with `dtctl get dashboard <id> -o json --plain | grep isPrivate` after applying.
