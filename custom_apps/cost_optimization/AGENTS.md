...
## Common Tasks
- **Add Route/Tab**: Update `NAV` and `Routes` in [ui/app/App.tsx](ui/app/App.tsx) (hand-rolled nav, not `AppHeader` — see the note in that file)
- **Query Data**: Add a DQL builder function to [ui/app/lib/queries.ts](ui/app/lib/queries.ts), then call it via `useCostOptQuery` ([ui/app/hooks/useCostOpt.ts](ui/app/hooks/useCostOpt.ts))
- **Style Components**: Import from `@dynatrace/strato-design-tokens/{colors,borders,box-shadows}` for design tokens; data-channel colors (per-metric constants, not tokens) live in [ui/app/lib/colors.ts](ui/app/lib/colors.ts)

## Project-Specific Notes

- **Every DQL query in `lib/queries.ts` was run via `dtctl query` before being wired into a component.** This app's target environment (`ditmar`) has no AWS/Azure/GCP entities monitored, so the Cloud tab's queries were instead verified against `demo-live` (which has real cloud entities) and then shipped as-is — they render `DataTable.EmptyState` in `ditmar` today, which is correct, not a bug.
- Kubernetes rightsizing math (`ui/app/lib/kubectl.ts`) hardcodes a 90th-percentile CPU / 100th-percentile (peak) memory / 10% buffer convention, matching the FinOps dashboard this app's Kubernetes tab was modeled on. These are constants, not a settings UI — add one only if a real need for tunable buffers comes up.
- Layout follows the `ncm` app (`ncm/app/ui/app` in this repo) — hand-rolled `NavLink` tab bar instead of `AppHeader`, `useXxxQuery` thin wrapper over `useDql`, DQL builders isolated in `lib/queries.ts`, stat-tile row + `DataTable` per tab. See the `dt-app-ui-design` Claude skill for the full pattern rationale.
